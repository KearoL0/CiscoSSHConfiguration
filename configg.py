...
TFTP_SERVER_IP = "192.168.1.100"  # Verander dit naar je TFTP-server IP
...
def configure_vlan(device, vlan_data):
    ...
    net_connect.enable()  # Inloggen in enable mode indien nodig
    net_connect.timeout = 120  # Verhoog de timeout voor trage verbindingen
...
if __name__ == "__main__":
    ...
    device = {
        ...
        'host': '192.168.1.10',  # Vervang dit met je daadwerkelijke switch IP
        'username': 'cisco',     # Vervang dit met je daadwerkelijke gebruikersnaam
        'password': 'cisco',     # Vervang dit met je daadwerkelijke wachtwoord
        'secret': 'cisco',       # Vervang dit met je enable wachtwoord indien nodig
    }

    # Lees de CSV
    file_path = 'D:\school\TREM 5\Infrastructure_Automation\week5-tdc\cisco-ssh-config\BST-D-1-242.csv'  # Het pad naar je CSV-bestand
    vlan_data = read_csv(file_path)

    # Configureer de switch met de VLAN gegevens
    configure_vlan(device, vlan_data)

    print("Configuratie is voltooid!")
